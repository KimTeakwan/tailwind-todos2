...
export default function FilterBar({
  ...
  return (
    <div className="flex items-center gap-2 mb-3">
      ...
      <div className="inline-flex items-center gap-2 ml-auto" role="tablist" aria-label="필터">
        {tab("all", "전체")}
        {tab("active", "미완료")}
        {tab("completed", "완료")}
        <motion.button onClick={onClearCompleted}
          ...
          whileTap={prefersReduced ? {} : { scale: 0.98 }}
          transition={{ type: "spring", stiffness: 400, damping: 25 }}
          className="px-3 py-2 text-sm transition border rounded-lg border-app bg-card hover:bg-accent-weak disabled:opacity-50 disabled:cursor-not-allowed"
          title="완료 항목 비우기"
        >
          완료 비우기
        </motion.button>
